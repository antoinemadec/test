...
def get_pod_thrust(p, target_x, target_y):
    ...
    # print(f"{p['name']=} {target_dist=} {p['cp_dist']=} {p['speed']=} {abs_deg=}", file=sys.stderr, flush=True)
    if abs_deg >= 90:
        t = 0
...
def print_pods(pods):
    ...
    worst_pod = 'pod1' if best_op == 'op2' else 'op2'
    ...
    for name in ["pod1", "pod2"]:
        ...
        if name == worst_pod:
            ...
            thrust = "SHIELD" if op_dist < 900 else get_pod_thrust(p, target_x, target_y)
        # racer
        else:
            op = pods[worst_op]
